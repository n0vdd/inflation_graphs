...
from .visualizer import EnhancedBrazilInflationVisualizer
    ...
    def _create_export_metadata(
        ...
        metadata = {
            ...
            'data_info': {
                ...
                'fetch_timestamp': dataset.retrieved_at.isoformat(),
                'total_observations': len(dataset.data_points),
                'date_range': {
                    'start': min(dp.date for dp in dataset.data_points).strftime('%Y-%m-%d'),
                    'end': max(dp.date for dp in dataset.data_points).strftime('%Y-%m-%d')
                },
